...
DnaStudioWorkspace::DnaStudioWorkspace(QWidget *parent) : Workspace(parent) {
    ...
    connect(m_map, &CircularMapView::selectionChanged, this, &DnaStudioWorkspace::onSelectionChanged);
    connect(m_map, &CircularMapView::documentEdited,   this, &DnaStudioWorkspace::onDocumentEdited);
...
void DnaStudioWorkspace::populateMenus(QMenuBar *mb) {
    // ---- File ----
    QMenu *file = mb->addMenu("File");
    QAction *open = file->addAction("Open…");
    open->setShortcut(QKeySequence::Open);
    connect(open, &QAction::triggered, this, &DnaStudioWorkspace::openFiles);
    file->addSeparator();
    QAction *quit = file->addAction("Quit");
    quit->setShortcut(QKeySequence::Quit);
    connect(quit, &QAction::triggered, qApp, &QApplication::quit);

    // ---- Edit (needs m_map, which exists by the time menus are built) ----
    ...
    connect(find, &QAction::triggered, this, &DnaStudioWorkspace::showFindBar);

    auto *findNext = edit->addAction("Find Next");
    findNext->setShortcut(QKeySequence::FindNext);          // Cmd/Ctrl+G
    connect(findNext, &QAction::triggered, m_map, &CircularMapView::nextHit);

    auto *findPrev = edit->addAction("Find Previous");
    ...
    // ---- Module menus (placeholders for now) ----
    for (const char *m : {"Tools", "Sequence", "Annotate && Predict"})
        mb->addMenu(m)->addAction("(placeholder)")->setEnabled(false);
}

void DnaStudioWorkspace::showFindBar() {
    m_findBar->show();
    m_findEdit->setFocus();
...
void DnaStudioWorkspace::doFind() {
    m_map->findMatches(m_findEdit->text(), m_findMm->value(), m_findBoth->isChecked());
}

// ----------------------------------------------------------- main toolbar ----

void DnaStudioWorkspace::buildMainToolBar() {
    ...
    connect(addBtn, &QToolButton::clicked, this, &DnaStudioWorkspace::openFiles);
    tb->addWidget(addBtn);
...
QWidget *DnaStudioWorkspace::buildSourceTree() {
    ...
    connect(m_tree, &QTreeWidget::currentItemChanged, this, &DnaStudioWorkspace::onFolderChanged);
    return m_tree;
}

// -------------------------------------------------------- document table -----

QWidget *DnaStudioWorkspace::buildDocumentTable() {
    ...
    connect(m_table, &QTableWidget::itemSelectionChanged, this, &DnaStudioWorkspace::onDocRowChanged);
    v->addWidget(m_table);
    return wrap;
}
...
QWidget *DnaStudioWorkspace::buildViewer() {
    ...
    connect(m_map, &CircularMapView::zoomChanged, this, &DnaStudioWorkspace::onZoomChanged);
    connect(m_map, &CircularMapView::hovered, this, &DnaStudioWorkspace::onHover);
    ...
    // Find wiring
    connect(m_findEdit, &QLineEdit::textChanged, this, &DnaStudioWorkspace::doFind);
    connect(m_findMm,   qOverload<int>(&QSpinBox::valueChanged), this, &DnaStudioWorkspace::doFind);
    connect(m_findBoth, &QCheckBox::toggled, this, &DnaStudioWorkspace::doFind);
...
QWidget *DnaStudioWorkspace::buildOptionsPanel() {
...
void DnaStudioWorkspace::buildStatusBar() {
...
void DnaStudioWorkspace::loadSampleLibrary() {
...
void DnaStudioWorkspace::populateTable(const QString &folder) {
...
void DnaStudioWorkspace::showDocument(const SequenceDocument &doc) {
...
void DnaStudioWorkspace::onFolderChanged() {
...
void DnaStudioWorkspace::onDocRowChanged() {
...
void DnaStudioWorkspace::openFiles() {
    const QStringList paths = QFileDialog::getOpenFileNames(
        this, "Open sequence files", QString(), SequenceIO::fileFilter());
    importPaths(paths);
}

void DnaStudioWorkspace::importPaths(const QStringList &paths, const QString &folderArg) {
...
void DnaStudioWorkspace::onHover(int base, QChar nt, int residue, const QString &aa) {
...
void DnaStudioWorkspace::onZoomChanged(int percent) {
